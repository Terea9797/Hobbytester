...
function validatePassword(pw: string) {
  ...
  const minLen = pw.length >= 8;           // frontend matches backend: min 8
  const withinMax = pw.length > 0 && pw.length <= 20;
  return { hasNumber, hasSpecial, minLen, withinMax, ok: hasNumber && hasSpecial && minLen && withinMax };
}
...
const LandingPage: React.FC<LandingProps> = ({
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage({ kind: "success", text: "Account created. Check your email to confirm." });
      setTab("login");
                ...
                <>
                  <form onSubmit={handleLogin} className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <button type="button" onClick={() => setShowForgot(true)} className="text-sm text-white/70 hover:text-white">
                        Forgot password?
                      </button>
                    </div>
  ...
  <li className={pwState.minLen ? "text-emerald-300" : ""}>• minimum 8 characters</li>
  ...
  <li className={pwState.withinMax ? "text-emerald-300" : ""}>• max 20 characters</li>
                  ...
                  <button
  type="submit"
  aria-disabled={!registerEnabled || busy}
  className={`group inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-orange-500 to-rose-500 px-6 py-3 text-base font-semibold text-white shadow-[0_25px_60px_-25px_rgba(255,90,31,0.65)] transition hover:from-orange-400 hover:to-rose-400 ${(!registerEnabled || busy) ? "opacity-50 cursor-not-allowed" : ""}`}
  title={!registerEnabled ? "Please fix the form errors" : (busy ? "Working..." : "")}
>
  Create Account
</button>
